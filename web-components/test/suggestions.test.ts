...
describe('Monaco AQL Editor Integration Tests', () => {
  ...
  async function getSuggestionsAtPosition(text: string, line: number = 1, column?: number): Promise<monaco.languages.CompletionItem[]> {
    ...
    const position = column !== undefined 
      ? { lineNumber: line, column: column }
      : { lineNumber: line, column: model.getLineLength(line) + 1 };
    ...
    const result = await provider.provideCompletionItems(model, position, { triggerKind: monaco.languages.CompletionTriggerKind.Invoke, triggerCharacter: undefined }, new monaco.CancellationToken());
  ...
  const testCases = [
    ...
    },
    {
      ...
      input: 'spans ',
      expectedContains: ['limit', 'stats', 'timechart'],
    },
    {
    ...
    {
      name: 'Stats segment - shows aggregation functions',
      input: 'spans | stats ',
      expectedContains: ['count', 'avg', 'sum', 'max', 'min'],
    },
    {
      name: 'After BY - shows fields for grouping',
      input: 'spans | stats count() by ',
      expectedContains: ['timestamp', 'id', 'status_code'],
    },
    {
  ...
  testCases.forEach((testCase) => {
    test(testCase.name, async () => {
      ...
      for (const expected of testCase.expectedContains) {
        const found = suggestionLabels.some(label => 
          typeof label === 'string' ? label.includes(expected) : false
        );
        
        expect(
          found,
          `Expected "${expected}" in suggestions but not found. Suggestions: ${suggestionLabels.join(', ')}`
        ).toBe(true);
      }
    });
  });
  ...
  test('Typing simulation - schema completion', async () => {
    ...
    expect(suggestionLabels.some(label => 
      typeof label === 'string' ? label.includes('spans') : false
    )).toBe(true);
  });
  ...
  test('Typing simulation - field completion after pipe', async () => {
    ...
    expect(suggestionLabels.some(label => 
      typeof label === 'string' ? label.includes('stats') : false
    )).toBe(true);
  });
  ...
  test('Suggestion insertion updates editor content', async () => {
    ...
    const schemaSuggestion = suggestions.find(s => 
      typeof s.label === 'string' && s.label === 'spans'
    );
