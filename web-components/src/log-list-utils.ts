...
export const SKELETON_COLUMN_WIDTHS = [
  'w-[17ch]', 
  'w-[16ch]', 
  'w-[25ch]', 
  'w-[12ch]', 
...
export const parseSummaryElement = (element: string): 
  | { type: 'plain'; content: string }
  | { type: 'formatted'; field: string; style: string; value: string } => {
...
export const formatTimestamp = (input: string): string => {
  ...
  return isValid(date)
    ? format(date, 'MMM dd HH:mm:ss') + `.${date.getUTCMilliseconds().toString().padStart(3, '0')}`
    : '';
};

export const lookupVecValue = <T = any>(vec: any[], colIdxMap: ColIdxMap, key: string): T => 
  get(vec, colIdxMap[key] ?? -1, '') as T;
...
export const getErrorClassification = (reqVec: any[], colIdxMap: ColIdxMap) => {
  ...
  return {
    ...
    hasErrors,
    className: clsx('w-1', {
      'bg-fillError-strong': hasErrors || errorStatus === 'ERROR',
      'bg-fillWarning-strong': !hasErrors && statusCode >= 400,
      'bg-fillBrand-weak status-indicator': !hasErrors && statusCode < 400
    })
  };
};
...
export const getColumnWidth = (column: string): string => 
  COLUMN_WIDTHS[column as keyof typeof COLUMN_WIDTHS] || 
  (column === 'id' ? '' : 'w-[16ch] shrink-0');

export const getSkeletonColumnWidth = (idx: number): string => 
  SKELETON_COLUMN_WIDTHS[idx % SKELETON_COLUMN_WIDTHS.length] + ' shrink-0';
