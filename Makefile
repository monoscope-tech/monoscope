...
live-reload:
	ghcid --command 'cabal repl monoscope --ghc-options="-w -j4 -Wno-error=unused-imports -Wno-error=unused-top-binds" --with-compiler=ghc-9.12.2' --test ':run Start.startApp' --warnings

live-test-reload:
	ghcid --command 'cabal repl lib:monoscope test/unit/Main.hs --ghc-options="-w -j4" --with-compiler=ghc-9.12.2' --test ':run main' --warnings
...
live-reload-doctests:
	ghcid --command 'cabal repl lib:monoscope --ghc-options="-w -j4" --with-compiler=ghc-9.12.2' --test ':! cabal test monoscope:doctests --ghc-options="-O0 -j8" --test-show-details=streaming'
