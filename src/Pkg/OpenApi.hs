...
--
-- The document scopes itself to whatever it was handed: one endpoint, one
-- host, or a whole project. Servers come from the entries rather than the
-- caller, so a project-wide document lists every host it observed instead of
-- claiming one.
buildSpec :: Text -> [(Text, Catalog.CatalogEntry)] -> AE.Value
buildSpec title entries =
  AE.object
    $ [ "openapi" .= ("3.1.0" :: Text)
      , "info"
          .= AE.object
    ...
    <> [("servers" :: AEK.Key) .= [AE.object ["url" .= ("https://" <> h)] | h <- hosts] | not (null hosts)]
  where
    hosts = ordNub $ sort [h | (_, e) <- entries, Just h <- [e.scope.host], not (T.null h)]
    -- Tags are the reader's table of contents. Within one host, the service
    -- that served each route is the useful split; across hosts it is the host,
    -- or every operation piles into one group.
    built = mapMaybe (operation (if length hosts > 1 then (.scope.host) else (.scope.service))) entries
...
operation
  :: (Catalog.CatalogEntry -> Maybe Text)
  -> (Text, Catalog.CatalogEntry)
  -> Maybe (Text, AEK.Key, AE.Value, [(AEK.Key, AE.Value)])
operation tagOf (hash, e) = do
