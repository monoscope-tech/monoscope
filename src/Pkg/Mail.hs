...
sampleAlert = \case
  ...
  LogPattern ->
    const
      $ LogPatternAlert
        { issueUrl = "https://example.com/test"
        , patternText = "Failed to connect to database: connection refused at <*>"
        , sampleMessage = Just "Failed to connect to database: connection refused at 10.0.0.1:5432"
        , logLevel = Just "error"
        , serviceName = Just "api-server"
        , sourceField = "body"
        , occurrenceCount = 42
        }
  LogPatternRateChange ->
    const
      $ LogPatternRateChangeAlert
        { issueUrl = "https://example.com/test"
        , patternText = "Request timeout after <*> ms for endpoint <*>"
        , sampleMessage = Just "Request timeout after 30000 ms for endpoint /api/users"
        , logLevel = Just "warning"
        , serviceName = Just "api-gateway"
        , direction = "spike"
        , currentRate = 150.0
        , baselineMean = 12.0
        , changePercent = 1150.0
        }
