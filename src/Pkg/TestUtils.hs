...
migrate db = do
  ...
  let q =
        [sql| UPDATE users.users SET is_sudo = true WHERE id = '00000000-0000-0000-0000-000000000000';
              ...
              VALUES ('00000000-0000-0000-0000-000000000000', 'Demo Project', 'Team', true, NULL, true, true)
              ON CONFLICT (id) DO UPDATE SET payment_plan = 'Team', active = true, deleted_at = NULL;
        ...
        void
          $ execute
            ...
            [sql| UPDATE users.users SET is_sudo = true WHERE id = '00000000-0000-0000-0000-000000000000';
                  ...
                  -- Paid plan ('Team'), matching the other demo-project seed sites. A free-tier
                  -- plan here makes manageMembersPostH's isFreeTier check deactivate non-owner members,
                  -- breaking member-management tests (the case-insensitive isFreeTier now matches 'FREE').
                  INSERT INTO projects.projects (id, title, payment_plan, active, deleted_at, weekly_notif, daily_notif)
                  VALUES ('00000000-0000-0000-0000-000000000000', 'Demo Project', 'Team', true, NULL, true, true)
                  ON CONFLICT (id) DO UPDATE SET payment_plan = 'Team', active = true, deleted_at = NULL;
    ...
    $ withResource pool \conn ->
      PGS.execute
        ...
        [sql|INSERT INTO projects.projects (id, title, description, payment_plan, active, deleted_at, weekly_notif, daily_notif)
         VALUES (?, 'Test Project', 'Test Description', 'Team', true, NULL, true, true)
         ON CONFLICT (id) DO UPDATE SET title = 'Test Project', description = 'Test Description', payment_plan = 'Team', active = true, deleted_at = NULL, weekly_notif = true, daily_notif = true|]
        (Only testProjectId)
