...
en =
  Map.fromList
    ...
    , ("onboarding.notif.telegram", "Telegram chat IDs")
    , ("onboarding.notif.telegram_help", "One per line (or comma-separated). Start a chat with your bot first; alerts are sent via the bot configured in TELEGRAM_BOT_TOKEN.")
    , ("onboarding.notif.webhooks", "Webhook URLs")
    , ("onboarding.notif.webhooks_help", "One per line. Each alert is POSTed as JSON to every URL.")
...
es =
  Map.fromList
    ...
    , ("onboarding.notif.telegram", "IDs de chat de Telegram")
    , ("onboarding.notif.telegram_help", "Uno por línea (o separados por coma). Primero inicia un chat con tu bot; las alertas se envían vía el bot configurado en TELEGRAM_BOT_TOKEN.")
    , ("onboarding.notif.webhooks", "URLs de webhooks")
    , ("onboarding.notif.webhooks_help", "Una por línea. Cada alerta se envía como POST JSON a cada URL.")
