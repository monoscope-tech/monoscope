  ...
  forM_ projectM \project -> Relude.when (Projects.isFreeTier project.paymentPlan) do
    ...
    Relude.when (warning || exceeded) do
      ...
      Relude.unless alreadyNotified do
        ...
        insertSystemLog ctx.env.enableTimefusionWrites ctx.env.timefusionUsesPgTypes $ mkSystemLog pid eventName sev bodyMsg attrs Nothing now
...
evaluateWithResults monitor startWall title total durationNs = do
  ...
  insertSystemLog ctx.env.enableTimefusionWrites ctx.env.timefusionUsesPgTypes otelLog{Telemetry.summary = generateSummary otelLog}
