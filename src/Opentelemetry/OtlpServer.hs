      ...
      let r =
            msgs' & V.map \(ackId, msg) -> do
              ...
              case resp of
                ...
                Right (ExportLogsServiceRequest a) -> (ackId, a)
          (_, rsLogs) = V.unzip r
          rLogs = join rsLogs
      projectIdsAndKeys <- dbtToEff $ projectIdsByProjectApiKeys $ V.toList $ getLogsApiKeys rLogs
      let results = V.map (\(ackid, a) -> (ackid, join $ V.map (convertToLog projectIdsAndKeys) a)) r
          (ackIds, logs) = V.unzip results
      ...
      let r =
            msgs' & V.map \(ackId, msg) -> do
              ...
              case res of
                ...
                Right (ExportTraceServiceRequest a) -> (ackId, a)
          (_, rsSpans) = V.unzip r
          rSpans = join rsSpans
      projectIdsAndKeys <- dbtToEff $ projectIdsByProjectApiKeys $ V.toList $ getApiKeys rSpans
      let results = V.map (\(ackId, a) -> (ackId, join $ V.map (convertToSpan projectIdsAndKeys) a)) r
          (ackIds, spansVec) = V.unzip results
          spans = join spansVec
          apitoolkitSpans =
            V.map
              ( \s ->
...
getLogsApiKeys :: V.Vector ResourceLogs -> V.Vector Text
getLogsApiKeys logs = V.catMaybes $ anyValueToString <$> val
  where
    val = V.catMaybes $ V.map (\lg -> lg.resourceLogsResource >>= \r -> find (\kv -> kv.keyValueKey == "at-project-key") r.resourceAttributes >>= (.keyValueValue) >>= (.anyValueValue)) logs


getApiKeys :: V.Vector ResourceSpans -> V.Vector Text
getApiKeys resourceSpans = V.catMaybes $ anyValueToString <$> val
  where
    val = V.catMaybes $ V.map (\sp -> sp.resourceSpansResource >>= \r -> find (\kv -> kv.keyValueKey == "at-project-key") r.resourceAttributes >>= (.keyValueValue) >>= (.anyValueValue)) resourceSpans
...
convertToLog :: V.Vector (UUID.UUID, Text) -> ResourceLogs -> V.Vector Telemetry.LogRecord
convertToLog pidsKeys resourceLogs = join $ V.map (convertScopeLog pidsKeys resourceLogs.resourceLogsResource) resourceLogs.resourceLogsScopeLogs


convertScopeLog :: V.Vector (UUID.UUID, Text) -> Maybe Resource -> ScopeLogs -> V.Vector Telemetry.LogRecord
convertScopeLog pidsKeys resource sl = V.catMaybes $ V.map (convertLogRecord pidsKeys resource sl.scopeLogsScope) sl.scopeLogsLogRecords
...
convertScopeSpan :: V.Vector (UUID.UUID, Text) -> Maybe Resource -> ScopeSpans -> V.Vector Telemetry.SpanRecord
convertScopeSpan pidsKeys resource sl = V.catMaybes $ V.map (convertSpanRecord pidsKeys resource sl.scopeSpansScope) sl.scopeSpansSpans


-- Convert ExportTraceServiceRequest to [Trace]
convertToSpan :: V.Vector (UUID.UUID, Text) -> ResourceSpans -> V.Vector Telemetry.SpanRecord
convertToSpan pidsKeys resourceSpans = join $ V.map (convertScopeSpan pidsKeys resourceSpans.resourceSpansResource) resourceSpans.resourceSpansScopeSpans
...
convertLogRecord :: V.Vector (UUID.UUID, Text) -> Maybe Resource -> Maybe InstrumentationScope -> LogRecord -> Maybe Telemetry.LogRecord
convertLogRecord pidsKeys resource scope lr =
  case pid of
    Just (pid', _) ->
      Just $
        Telemetry.LogRecord
    ...
    Nothing -> Nothing
  where
    apKey = maybe "" (fromMaybe "" . anyValueToString) $ resource >>= \r -> find (\kv -> kv.keyValueKey == "at-project-key") r.resourceAttributes >>= (.keyValueValue) >>= (.anyValueValue)
    pid = find (\(_, k) -> k == apKey) pidsKeys


convertSpanRecord :: V.Vector (UUID.UUID, Text) -> Maybe Resource -> Maybe InstrumentationScope -> Span -> Maybe Telemetry.SpanRecord
