...
bulkInsertEndpoints eps = Hasql.interpExecute_
...
endpointRequestStatsByProject pid _ackd _archived pHostM sortM searchM page requestType period = V.fromList <$> Hasql.interp query
  where
    ...
    query = "WITH combined AS (\
      ...
      \        LEAST(width_bucket(EXTRACT(EPOCH FROM timestamp), EXTRACT(EPOCH FROM " <> seriesStartSql <> "::timestamptz), EXTRACT(EPOCH FROM NOW()), " <> numBucketsSql <> "), " <> numBucketsSql <> ") AS bucket_idx,\
      \        COUNT(*)::int AS cnt\
      \ FROM otel_logs_and_spans\
      \ WHERE project_id = " <> [HI.sql|#{pid}::text|] <> " AND attributes___http___request___method IS NOT NULL" <> hostFilter
      ...
      \ CROSS JOIN generate_series(1, " <> numBucketsSql <> ") AS s(idx)\
      ...
      \ WHERE enp.project_id=" <> [HI.sql|#{pid}|] <> " AND enp.outgoing=" <> [HI.sql|#{isOutgoing}|] <> pHostQuery <> search
      <> " ORDER BY " <> rawSql orderBy <> ", url_path ASC OFFSET " <> [HI.sql|#{offset}|] <> " LIMIT 30"
...
WITH endpoint_hosts AS (
    ...
    WHERE project_id = |] <> [HI.sql|#{pid} |] <> rawSql [text| AND host != '' AND $endpointFilter
), combined AS (
    SELECT $hostExpr AS host,
           ...
           COUNT(*)::int AS cnt
    FROM otel_logs_and_spans
    WHERE project_id = |] <> [HI.sql|#{pid}::text AND |] <> timeRangeSql <> [HI.sql| AND attributes___http___request___method IS NOT NULL AND kind IN (CASE WHEN #{isOutgoing} THEN 'client' ELSE 'server' END, CASE WHEN #{isOutgoing} THEN NULL ELSE 'internal' END) GROUP BY host, bucket_idx
), host_stats AS (
...
countEndpointInbox pid host requestType = do
  let showCountBaseOnRequestType = case requestType of
        ...
        _ -> "ep.outgoing =  false"
  fromMaybe 0 <$> Hasql.interpOne (
    [HI.sql|SELECT coalesce(COUNT(*)::INT, 0)
        FROM apis.endpoints enp
        LEFT JOIN apis.issues ann ON (ann.issue_type = 'api_change' AND ann.endpoint_hash = enp.hash)
        WHERE enp.project_id = #{pid} AND |] <> rawSql showCountBaseOnRequestType <> [HI.sql| AND ann.id IS NOT NULL AND ann.acknowledged_at IS NULL AND host = #{host}|])
...
getUnmergedEndpoints pid =
  Hasql.interp [HI.sql| SELECT hash, method, host, url_path FROM apis.endpoints
...
setEndpointCanonical triples =
  Hasql.interpExecute [HI.sql| UPDATE apis.endpoints SET canonical_hash = u.chash, canonical_path = u.cpath
...
insertCanonicalEndpoints rows = Hasql.interpExecute_
...
getUnembeddedEndpoints pid =
  Hasql.interp [HI.sql| SELECT id, hash, url_path FROM apis.endpoints
...
updateEndpointEmbeddings pairs =
  Hasql.interpExecute [HI.sql| UPDATE apis.endpoints SET
...
assignEndpointsToCanonical pairs =
  Hasql.interpExecute [HI.sql| UPDATE apis.endpoints e SET canonical_hash = canon.hash
...
getMergedEndpointPairs pid =
  Hasql.interp [HI.sql| SELECT hash, canonical_hash FROM apis.endpoints
