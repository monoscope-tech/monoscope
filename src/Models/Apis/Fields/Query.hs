...
fieldsByEndpointHashes :: Projects.ProjectId -> Vector Text -> PgT.DBT IO (Vector Field)
fieldsByEndpointHashes pid hashes = query Select q (pid, hashes)
 where
  q =
    [sql|
      SELECT key, field_type, field_type_override, format, description,
             key_path, field_category, hash,format_override, endpoint_hash
      FROM apis.fields
