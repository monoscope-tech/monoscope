...
getUnembeddedErrorPatterns pid = PG.query
...
getUnembeddedLogPatterns pid = PG.query
...
updateErrorEmbeddings pairs = PG.execute
  ...
  where (ids, embs) = unzip $ map (\(eid, e) -> (eid, PGArray e)) pairs


updateLogEmbeddings :: DB es => [(LogPatternId, [Float])] -> Eff es Int64
updateLogEmbeddings [] = pure 0
updateLogEmbeddings pairs = PG.execute
...
assignErrorsToCanonical pairs = PG.execute
...
unmergeErrorPattern eid = PG.execute
  [sql| UPDATE apis.error_patterns SET merge_override = TRUE, canonical_id = NULL WHERE id = ? |] (Only eid)


unmergeLogPattern :: DB es => LogPatternId -> Eff es Int64
unmergeLogPattern lid = PG.execute
  [sql| UPDATE apis.log_patterns SET merge_override = TRUE, canonical_id = NULL WHERE id = ? |] (Only lid)
...
getErrorPatternMemberCount eid = fromMaybe 0 . fmap fromOnly . listToMaybe <$> PG.query
  [sql| SELECT COUNT(*)::int FROM apis.error_patterns WHERE canonical_id = ? |] (Only eid)


getLogPatternMemberCount :: DB es => LogPatternId -> Eff es Int
getLogPatternMemberCount lid = fromMaybe 0 . fmap fromOnly . listToMaybe <$> PG.query
  [sql| SELECT COUNT(*)::int FROM apis.log_patterns WHERE canonical_id = ? |] (Only lid)
