...
import Relude
import Test.Hspec
import Database.PostgreSQL.Entity.DBT (withPool, execute, QueryNature(Insert))
import Data.Maybe
...
spec = aroundAll TmpPg.withSetup $ describe "Models.Projects.Swaggers" $ do
  ...
  describe "swaggersByProject" $
    it "should retrieve all Swaggers for a given Project" $ \pool -> do
      result <- withPool pool $ do
        createSwagger (ProjectId UUID.nil) (UserId UUID.nil)  swaggerJson1
        createSwagger (ProjectId UUID.nil) (UserId UUID.nil)  swaggerJson2
  ...
  describe "updateSwagger" $
    it "should update the Swagger JSON of a Swagger" $ \pool -> do
      result <- withPool pool $ do
        updateSwagger (swaggerId.toText) swaggerJson2
        getSwaggerById (swaggerId.toText)
