module ProcessMessageSpec (spec, testAuthContext, msg1, msg2, runTestBackground, convert) where
...
spec = aroundAll TmpPg.withSetup do
  describe "process request to db" do
    it "test processing raw request message string" \pool -> do
      ...
      resp <- runTestBackground authCtx $ processMessages authCtx.config msgs authCtx.projectCache
      resp `shouldBe` ["m1"]
    ...
    it "We should expect 2 endpoints, albeit unacknowleged." \pool -> do
      withPool pool $ execute Select [sql|CALL apis.refresh_request_dump_views_every_5mins(0, '{}')|] ()
