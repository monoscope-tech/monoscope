...
testAuthContext :: (Pool Connection, Hasql.Pool) -> IO (Config.AuthContext)
testAuthContext (pool, hPool) = do
  ...
  pure $ Config.AuthContext{env = def :: Config.EnvConfig, pool = pool, jobsPool = pool, hasqlPool = hPool, projectCache = projectCache, config}
...
spec = aroundAll TestUtils.withSetup do
  describe "process request to db" do
    it "test processing raw request message string" \pool -> do
      ...
      Right resp <- TestUtils.runTestBackground authCtx $ processMessages msgs
      resp `shouldBe` ["m1"]
    ...
    it "should save the request" \pool -> do
      ...
      authCtx <- testAuthContext pool
      Right resp <- TestUtils.runTestBackground authCtx $ processRequestMessages msgs
      resp `shouldBe` ["m1", "m2"]

    it "should be able to query request dumps that include the added requests" \(pool, _) -> do
    ...
    it "We should expect 2 endpoints, albeit unacknowleged." \(pool, _) -> do
