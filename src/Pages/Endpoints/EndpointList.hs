...
endpointList enps = do
  div_ [class_ "container mx-auto relative  px-4 pt-10 pb-24 h-full overflow-y-scroll"] $ do
    -- modal
    div_ [style_ "background-color: rgba(0,0,0,0.5);z-index:99999", class_ "fixed flex items-center justify-center top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full", id_ "staticModal", tabindex_ "-1"] $ do
      div_ [class_ "relative w-[500px] max-h-full", style_ "width: min(90vw, 750px)"] $ do
        -- Modal content
        div_ [class_ "relative bg-white rounded-lg shadow"] $ do
          div_ [class_ "flex items-start justify-between p-4 border-b rounded-t"] $ do
            ...
            button_ [type_ "button", class_ "btn bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"] "Close"
          -- Modal body
          div_ [class_ "p-6 space-y-6"] $ do
            textarea_ [style_ "height:65vh;resize:none", class_ "w-full border outline-none p-4", placeholder_ "Paste swagger here"] ""
          -- Modal footer
          div_ [class_ "flex w-full justify-between items-center p-6 space-x-2 border-t border-gray-200 rounded-b"] $ do
            button_ [type_ "button", class_ "btn"] "Close"
    ...
    div_ [class_ "flex flex-col justify-between"] $ do
      div_ [class_ "flex w-full justify-between my-3 px-2"] $ do
        ...
        button_ [class_ "place-content-center text-md btn btn-primary"] "Upload swagger"
      -- search
