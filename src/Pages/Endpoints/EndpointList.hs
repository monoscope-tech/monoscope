      ...
      div_ [class_ "card-round p-5"] $ do
        div_ [class_ "w-full flex flex-row m-3"] $ do
          div_ [class_ "flex rounded-xl bg-white py-2 px-3 flex-row w-3/4 border-solid border border-gray-200 h-10"] $ do
            ...
            input_ [type_ "text", oninput_ "search(event)", class_ "dataTable-search w-full h-full p-2 text-sm text-gray-400 font-normal focus:outline-none", placeholder_ "Search endpoints..."]
  ...
  script_
    [text|
          ...
          function search(event) {
             ...
             const rows = document.querySelectorAll(".endpoint_row")
             if(rows) {
                for(let row of Array.from(rows)) {
                  const method = row.querySelector('[data-enp-urlMethod]');
                  const path = row.querySelector('[data-enp-urlPath]');
                  let methodData = ""
                  let pathData = ""
                  if(method) { 
                      methodData = method.getAttribute('data-enp-urlMethod')
                    }
                  if(path) {
                      pathData = path.getAttribute('data-enp-urlPath')
                    }
                  if(!(pathData.toLowerCase().includes(val) || methodData.toLowerCase().includes(val))) {
