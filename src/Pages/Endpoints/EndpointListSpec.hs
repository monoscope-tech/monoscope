...
spec = aroundAll withTestResources do
  describe "Check Endpoint List" do
    it "should return an empty list" \TestResources{..} -> do
      ...
      enp <-
        toServantResponse trATCtx trSessAndHeader trLogger $ EndpointList.endpointListGetH testPid Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing
      case enp of
    ...
    it "should return inbox endpoints list and acknowledge endpoints" \TestResources{..} -> do
      ...
      enp <-
        toServantResponse trATCtx trSessAndHeader trLogger $ EndpointList.endpointListGetH testPid Nothing Nothing (Just "Inbox") Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing
      case enp of
        EndpointList.EndpointsListPage (PageCtx _ (ItemsList.ItemsPage _ enpList)) -> do
