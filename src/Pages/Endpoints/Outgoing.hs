...
outgoingList' pid sortV hostsEvents = div_ [class_ "col-span-5 bg-white divide-y ", id_ "anomalyListForm"] $ do
  div_ [class_ "flex py-3 px-3 gap-8 items-center  bg-gray-50 overflow-hidden"] do
    div_ [class_ " grow flex flex-row gap-2 font-bold"] do
      "HOST"
...
renderOutgoing pid hostsEvents = do
  div_ [class_ "flex flex-col", id_ "hosts_container"] do
    forM_ hostsEvents $ \host -> do
      div_ [class_ "flex py-4 gap-8 items-center hover:bg-gray-100 host"] do
