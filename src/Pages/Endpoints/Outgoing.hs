...
outgoingList' pid sortV hostsEvents = do
  form_ [class_ "col-span-5 bg-white divide-y", id_ "anomalyListForm"] $ do
    div_ [class_ "flex py-3 gap-8 items-center  bg-gray-50"] do
      ...
      div_ [class_ " grow flex flex-row gap-2"] do
        button_ [class_ "btn btn-sm btn-outline border-black hover:shadow-2xl"] "HOST"
      let currentSortTitle = maybe "Events" fst3 $ find (\(_, _, identifier) -> identifier == sortV) sortOptions
      div_ [class_ "relative inline-block"] do
...
renderOutgoing pid sortV hostsEvents = do
  div_ [class_ ""] $ do
    forM_ hostsEvents $ \host -> do
      div_ [class_ "flex py-4 gap-8 items-center"] do
        div_ [class_ "h-4 flex space-x-3 w-8"] do
          a_ [class_ " w-2 h-full"] ""
        -- input_ [term "aria-label" "Select Issue", type_ "checkbox"]
        div_ [class_ "space-y-3 grow"] do
          div_ [class_ "space-x-3"] do
            a_ [class_ "inline-block font-bold text-red-700 space-x-2"] $ do
              a_ [class_ "text-blue-500 hover:text-slate-600", href_ $ "/p/" <> pid.toText <> "/log_explorer?query=host%3D%3D" <> "\"" <> host.host <> "\""] $ "View Log"
              a_ [class_ "", href_ $ "/p/" <> pid.toText <> "/endpoints?host=" <> host.host] do
                span_ [class_ " inconsolata text-base text-slate-700"] $ toHtml (T.replace "http://" "" $ T.replace "https://" "" host.host)
