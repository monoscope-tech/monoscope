    ...
    nav_ [class_ "mt-5 flex flex-col gap-2.5"] do
      ...
      menu project.id & mapM_ \(mTitle, mUrl, fIcon) -> do
        ...
        a_
          ...
          , class_ $ "group-has-[#sidenav-toggle:checked]/pg:px-4 gap-3 py-2 flex no-wrap shrink-0  justify-center group-has-[#sidenav-toggle:checked]/pg:justify-start items-center rounded-lg border-slate-300 hover:shadow hover:border  overflow-x-hidden h-9" <> activeCls
          ]
          do
