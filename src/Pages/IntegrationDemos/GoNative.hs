...
goNativeGuide :: Text -> Html ()
goNativeGuide apikey = do
  section_ [class_ "flex flex-col gap-10"] do
    div_ [class_ "w-full flex flex-col gap-2"] do
      ...
      p_ [class_ "text-gray-600 font-medium flex items-center gap-1"] do
        "Install the APItoolkit go SDK using "
        codeEmphasis "go get:"
    ...
    div_ [class_ "w-full flex flex-col gap-2", id_ "requests-monitoring"] do
      ...
      p_ [class_ "text-gray-600 font-medium"] "Configure and initialize the SDK to start monitoring incoming request to your native go server"
