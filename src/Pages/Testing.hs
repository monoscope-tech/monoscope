  ...
  div_ [class_ "w-full"] do
    ...
    div_ [class_ "w-full mt-4 max-w-6xl mx-auto"] do
      div_ [class_ "flex justify-between border-b py-2 items-center"] do
        ...
        button_ [
          class_ "text-white rounded bg-blue-500 px-4 py-2 flex items-center gap-2", [__|on click remove .hidden from #col-modal|]] do
          faIcon_ "fa-plus" "fa-light fa-plus" "h-6 w-6"
          "Collection"
      div_ [class_ "w-full grid grid-cols-2 gap-4 mt-4"] do
        collectionCard
        collectionCard
        collectionCard
...
collectionCard :: Html ()
collectionCard = do
  a_ [href_ ("/p/testing/" <> toText "test_id")] $ do
    div_ [class_ "rounded-xl border text-gray-700 h-full shadow"] $ do
      div_ [class_ "flex flex-col p-4 gap-5"] $ do
        ...
        div_ [class_ "flex flex-col w-full gap-2"] $ do
          h3_ [class_ "font-semibold tracking-tight text-xl"] $ toHtml "Testing user profile"
          p_ [class_ "text-sm text-gray-500 break-words"] $ toHtml "user profile for making sure everything works"
...
modal = do
  div_ [
    class_ "fixed inset-0 z-50 w-screen hidden overflow-y-auto bg-gray-300 bg-opacity-50", id_ "col-modal", [__|on click add .hidden to #col-modal|]] $ do
    div_ [class_ "flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0"] $ do
      div_ [class_ "relative transform overflow-hidden rounded border bg-white text-left transition-all my-8 w-full max-w-2xl",  [__|on click halt|]] $ do
        div_ [class_ "bg-white pb-4"] $ do
          ...
          div_ [class_ "px-6 mt-4 items-start flex flex-col gap-5 text-gray-600"] $ do
            div_ [class_ "flex flex-col gap-1 w-full"] $ do
              ...
              input_ [type_ "text", name_ "title", id_ "title",
            ...
            div_ [class_ "flex flex-col gap-1 w-full"] $ do
              ...
              textarea_ [type_ "text", name_ "desc", id_ "desc",
        ...
        div_ [class_ "px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 border-t mt-4"] $ do
          button_ [type_ "button", [__|on click add .hidden to #col-modal|],
                   class_ "inline-flex w-full justify-center rounded-md bg-blue-500 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-600 sm:ml-3 sm:w-[100px]"] "Save"
          button_ [type_ "button",
                   class_ "mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-[100px]"] "Cancel"
