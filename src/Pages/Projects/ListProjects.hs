...
listProjectsBody projects = do
  section_ [id_ "main-content", class_ "container mx-auto p-6 pb-36 overflow-y-scroll  h-full"] do
    div_ [class_ "flex justify-between mb-6"] do
      h2_ [class_ "text-textStrong text-2xl font-medium"] "Projects"
...
projectItem_ project = li_ do
  a_ [href_ ("/p/" <> project.id.toText), class_ "block hover:bg-fillWeaker"] do
    div_ [class_ "px-4 py-4 flex items-center sm:px-6"] do
      div_ [class_ "min-w-0 flex-1 sm:flex sm:items-center sm:justify-between"] do
        div_ [class_ "truncate"] do
          div_ [class_ ""] do
            p_ [class_ "block font-medium text-textBrand truncate py-2"] $ toHtml project.title
            p_ [class_ "block shrink-0 font-normal text-textWeak"] $ toHtml project.description
          div_ [class_ "mt-2 flex"] do
            div_ [class_ "flex items-center  text-textWeak"] do
              small_ do
                span_ "Created on "
      ...
      div_ [class_ "ml-5 shrink-0 text-textWeak"] do
