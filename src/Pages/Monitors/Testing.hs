...
import Fmt.Internal.Core (fmt, pretty)
...
import Models.Projects.ProjectMembers (Team (slack_channels), discord_channels)
...
import Utils (checkFreeTierExceeded, faSprite_, getDurationNSMS, listToIndexHashMap, lookupValueInt, lookupVecIntByKey, lookupVecTextByKey, lookupVecValueByKey, parseTime, toUriStr)
  ...
  case alertM of
    Just alert -> do
      ...
      discordChannels <- case discordDataM of
        ...
        Nothing -> return []
      let queryText = "kind==\"alert\" and context___trace_id==\"" <> monitorId <> "\""
          queryAST = case parseQueryToAST queryText of
            Right ast -> ast
            Left _ -> []
      tableAsVec <- RequestDumps.selectLogTable pid queryAST queryText Nothing (Nothing, Nothing) ["timestamp", "status_message", "body"] Nothing Nothing
      let tableData = case tableAsVec of
            Right tbl -> tbl
            Left _ -> ([], [], 0)
      ...
      addRespHeaders $ PageCtx bwconf' $ unifiedOverviewPage pid alert currTime teams' slackDataM discordDataM tableData
...
unifiedOverviewPage :: Projects.ProjectId -> Monitors.QueryMonitorEvaled -> UTCTime -> V.Vector ManageMembers.Team -> Maybe Slack.SlackData -> Maybe Slack.DiscordData -> (V.Vector (V.Vector AE.Value), [Text], Int) -> Html ()
unifiedOverviewPage pid alert currTime teams slackDataM discordDataM tableData = do
