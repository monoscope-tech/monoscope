    ...
    then do
      ...
      (project, Right tableAsMaps) <- liftIO
        $ withPool pool do
          ...
          tableAsMaps <- RequestDumps.selectLogTable pid query cursorM' (fromD, toD) cols
      ...
      let (requestMaps, colNames, requestsCount) = tableAsMaps
          ...
          page = ApiLogsPageData{pid, resultCount = requestsCount, requestMaps, cols = colNames, reqChartTxt, nextLogsURL, resetLogsURL, currentRange}

      case (layoutM, hxRequestM, hxBoostedM) of
        (Just "loadmore", Just "true", _) -> pure $ logItemRows_ pid requestMaps (curateCols colNames) nextLogsURL
        (Just "resultTable", Just "true", _) -> pure $ resultTable_ page
        (Just "all", Just "true", _) -> pure $ apiLogsPage page
        _ -> do
          let bwconf =
                (def :: BWConfig)
                  { sessM = Just sess
      ...
      div_ [class_ "flex-1 "] do
        div_ [class_ "pl-3 py-1 flex flex-row justify-between"]
          $ a_ [class_ "cursor-pointer inline-block pr-3 space-x-2 bg-blue-50 hover:bg-blue-100 blue-800 p-1 rounded-md", [__|on click toggle .hidden on #reqsChartParent|]] do
            ...
            span_ [] "toggle chart"
        reqChart page.reqChartTxt False
...
resultTable_ page = table_ [class_ " table table-sm table-pin-rows table-pin-cols", style_ "height:1px", id_ "resultTable"] do
  ...
  let curatedCols = curateCols page.cols
  thead_ $ tr_ $ forM_ curatedCols $ logTableHeading_ page.pid isLogEventB
  tbody_ [id_ "log-item-table-body"] $ logItemRows_ page.pid page.requestMaps curatedCols page.nextLogsURL


curateCols :: [Text] -> [Text]
curateCols cols = sortBy sortAccordingly filteredCols
  where
    defaultSummaryPaths = ["errors_count", "host", "status_code", "method", "url_path", "request_type"]
    isLogEventB = isLogEvent cols
    filteredCols = filter (\c -> not isLogEventB || c `notElem` defaultSummaryPaths) cols
   ...
   in ( status
      ...
      , if errCount > 0 || status >= 400
          then " w-1 bg-red-500 "
            ...
            if expandedSection
              then " w-1 bg-blue-200 "
              else " w-1 bg-transparent status-indicator "
      )
...
logTableHeadingWrapper_ pid title child = td_
  ...
  $ div_
    [class_ "dropdown"]
    do
      div_ [tabindex_ "0", role_ "button"] child
      ul_ [tabindex_ "0", class_ "dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box "] do
        ...
        li_ $ a_ [hxGet_ $ "/p/" <> pid.toText <> "/log_explorer"
          , hxPushUrl_ "true"
          , hxVals_ $ "js:{query:params().query,cols:toggleNamedColumnToSummary('" <> title <> "')(event),layout:'resultTable'}"
...
jsonTreeAuxillaryCode pid = do
    ...
    div_ [id_ "log-item-context-menu", class_ "log-item-context-menu text-sm origin-top-right absolute left-0 mt-2 w-56 rounded-md shadow-md shadow-slate-300 bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 focus:outline-none z-10", role_ "menu", tabindex_ "-1"] do
      div_ [class_ "py-1", role_ "none"] do
        a_
          ...
          , hxGet_ $ "/p/" <> pid.toText <> "/log_explorer"
          , hxPushUrl_ "true"
          , hxVals_ "js:{query:params().query,cols:toggleColumnToSummary(event),layout:'resultTable'}"
  ...
  script_
    ...
    var toggleNamedColumnToSummary = (namedCol)=>(e)=>{
      const cols = (params().cols??"").split(",").filter(x=>x!="");
