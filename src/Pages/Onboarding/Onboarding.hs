  ...
  stepData <- case onboardingStep of
    ...
    "NotifChannel" -> do
      ...
      let phone = fromMaybe "" project.notifyPhoneNumber
          ...
          slackUrl = "https://slack.com/oauth/v2/authorize?client_id=" <> appCtx.config.slackClientId <> "&scope=chat:write,commands,incoming-webhook,files:write,app_mentions:read,channels:history,groups:history,im:history,mpim:history,chat:write.public&user_scope=&redirect_uri=" <> slackRedirectUri <> "&state=" <> pid.toText <> "__onboarding"
