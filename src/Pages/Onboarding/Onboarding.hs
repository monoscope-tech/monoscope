module Pages.Onboarding.Onboarding (onboardingGetH, OnboardingInfoForm (..)) where
...
onboardingGetH :: Projects.ProjectId -> ATAuthCtx (RespHeaders (PageCtx (Html ())))
onboardingGetH projectId = do
  (sess, project) <- Sessions.sessionAndProject projectId
...
stepIndicator step title =
  div_ [class_ "flex-col gap-4 flex w-full"] $ do
    div_ [class_ "flex-col gap-2 flex w-full"] $ do
      ...
      when (step > 1) $ do
        div_ [class_ "flex items-center gap-3 flex text-brand w-full mt-2"] $ do
