...
spec = aroundAll withTestResources do
  describe "Check Log Page" do
    it "should return an empty list" \TestResources{..} -> do
      pg <-
        toServantResponse trATCtx trSessAndHeader trLogger $ Log.apiLogH testPid Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing
    ...
    it "should return log items" \TestResources{..} -> do
      ...
      case pg of
        Log.LogPage (PageCtx _ content) -> do
          ...
          pg2 <-
            toServantResponse trATCtx trSessAndHeader trLogger $ Log.apiLogH testPid Nothing Nothing Nothing cur Nothing Nothing Nothing (Just "loadmore") Nothing Nothing Nothing Nothing (Just "true") Nothing
          case pg2 of
