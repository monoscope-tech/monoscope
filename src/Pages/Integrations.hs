...
integrationSettingGetH pid = do
  ...
  let bwconf = (def :: BWConfig){sessM = sess.persistentSession, currProject = projM, pageTitle = "Integration"}
  pure $ bodyWrapper bwconf $ integrationBody pSess appCtx.config True createProj  (Just proj.notificationsChannel) slackInfo
...
----------------------------------------------------------------------------------------------------------
-- integrationBody is the core html view

integrationBody :: Sessions.PersistentSession -> EnvConfig -> Bool -> CreateProjectForm -> Maybe (V.Vector Projects.NotificationChannel) -> Maybe SlackData -> Html ()
integrationBody sess envCfg isUpdate cp notifChannel slackData = do
  ...
  section_ [id_ "main-content", class_ "p-3 py-5 sm:p-6 overflow-y-scroll h-full"] do
    div_ [class_ "mx-auto", style_ "max-width:800px"] do
      h2_ [class_ "text-slate-700 text-3xl font-medium mb-5"] $ toHtml @String $ if isUpdate then "Integration" else "Create Project"
      when isUpdate do
