...
anomalySystemPrompt now =
  unlines
    ...
    , ""
    , "## Tool-Use Policy"
    , "- Analysis questions (\"What could cause this?\", \"Suggest a fix\") → answer DIRECTLY from the provided issue context. Do NOT call tools."
    , "- Chart / visualization requests (\"plot errors over time\", \"show a chart of...\") → build the KQL query directly from the schema. Do NOT call `get_schema` or `get_field_values`."
    , "- Tools are ONLY for fetching actual data values needed in the answer (e.g. \"top 5 services by error count\" where real numbers are required)."
    , "- The schema above is authoritative for all field names."
    , ""
    , "## Response Format"
    ...
    , "- Use only fields that appear in <schema>."
    , "- Never put timestamp filters in the query; set `time_range` instead."
    , "- Output JSON per the output_format spec — raw JSON only, no code fences."
    ]
