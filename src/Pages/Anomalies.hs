  ...
  users :: [Projects.User] <-
    if null userIds
      then pure []
        ...
        Hasql.interp [HI.sql| SELECT id, created_at, updated_at, deleted_at, active, first_name, last_name, display_image_url, email, is_sudo, phone_number, password_hash, company_name, company_size, found_us_from FROM users.users WHERE id = ANY(#{vUserIds}::uuid[]) |]
