...
drawerWithURLContent_ drawerId url trigger = div_ [class_ "drawer drawer-end inline-block w-auto"] do
  ...
  div_ [class_ "drawer-side fixed top-0 left-0 w-full h-full flex z-[10000] overflow-y-scroll ", style_ "position:fixed;width:100%;display:flex"] do
    ...
    div_ [style_ "width: min(90vw, 1200px)", class_ "bg-base-100 h-full clear-both overflow-y-scroll"] do
      ...
      div_ [class_ "bg-base-100 h-full overflow-y-auto p-4 flex flex-col", hxGet_ url, hxTrigger_ "intersect once", hxSwap_ "innerHTML"] $ span_ [class_ "loading loading-dots loading-md"] ""
