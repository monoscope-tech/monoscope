...
// Helper
const ws = whitespace.many(), ignoreWs = p => ws.then(p).then(ws);
...
const operator = ignoreWs(alt(str("=").map(() => "="), str("ANY"), str("EXISTS")));
...
// Expressions
const expression = seq(path, operator, alt(arrayValues.between(lb, rb), value, succeed(undefined))).map(
  ([p, op, v]) => (op === "EXISTS" ? [p, "EXISTS"] : [p, op, v])
);

// Conditions
const condition = lazy(() => alt(expression, not.then(condition).map(c => ({ NOT: c })), conditionPart));
...
console.log(JSON.stringify(parseQuery(query), null, 2));
