...
function throughputEChart(renderAt, data, gb, showLegend, showAxes, theme) {
  ...
  const resizeObserver = new ResizeObserver((_entries) =>
    window.requestAnimationFrame(() => myChart.resize())
  );
    ...
    if (param.value !== null && param.value[1] !== null) {
      result += `<div >
                            <div class="monospace flex flex-row space-between">
                                ...
                                <strong class="shrink pl-3 font-bold">${param.value[1]
        }</strong>
                              </div>
                        </div>`;
    }
...
function throughputEChartTable(
  renderAt,
  categories,
  data,
  gb,
...
const getErrorIndicator = () =>
  elt(
    ...
    {
      class:
        "bg-red-600 rounded-l h-full w-5 flex justify-center items-center rounded-r shrink-0 font-bold",
    },
    elt("span", {
      class:
        "text-white text-xs h-3 w-3 flex items-center justify-center rounded-full border border-white p-1",
    }, "!"),
  );
function flameGraphChart(data, renderAt, colorsMap) {
  ...
  const renderItem = (item, renderAt, rootVal) => {
    ...
    const text = elt(
      "span",
      { class: "text-black shrink-0 mr-4 text-xs" },
      item.name,
    );
    const [t, u] = formatDuration(item.value[2]);
    const tim = elt(
      "span",
      { class: "text-black text-xs shrink-0 ml-auto" },
      `${Math.floor(t)} ${u}`,
    );
...
function generateTimeIntervals(duration, target) {
  const container = document.querySelector("#" + target);
  container.innerHTML = "";
  const containerWidth = target === "waterfall-time-container"
    ? 550
    : container.offsetWidth - SCROLL_BAR_WIDTH;
  ...
  for (let i = 0; i < 10; i++) {
    ...
    unit = t === 0 ? "" : unit;
    intervals.push(`
              <div class="absolute bottom-0 text-gray-700 border-left overflow-x-visible" style="width: ${intervalWidth}px; left: ${i * intervalWidth
      }px;">
...
function buildWaterfall(spans, target, serviceColors, start, maxDuration) {
  ...
  spans.forEach((span) => {
    container.appendChild(
      buildTree(span, serviceColors, start, maxDuration, containerWidth),
    );
  });
}
...
function buildTree(span, serviceColors, start, rootVal, containerWidth) {
  ...
  span.children.forEach((child) => {
    childDiv.appendChild(
      buildTree(child, serviceColors, startCurr, rootVal, containerWidth),
    );
  });
  const text = elt("span", {
