...
class SuggestionService {
  ...
  parseInput = input => {
    ...
    for (const pattern of patterns) {
      ...
      return [field, opOrValue ?? '==', value ?? opOrValue ?? ''].map(x =>
        x?.trim()?.replace(/^["']|["']$/g, '') ?? '');
    }
    return [input.trim()];
  };
  ...
  async updateStateFromInput(input) {
    ...
    return {
      qs: {
        ...
        op: fieldSchema && validOps.some(validOp => input.includes(validOp)) ?
          validOps.find(validOp => input.includes(validOp)) : null
      },
      suggestions
    };
  }
  ...
  async handleSelection(type, value, currentQs) {
    if (type === 'field') {
      ...
      const suggestions = AUTOCOMPLETE_SUPPORTED_FIELDS.includes(baseField) ?
...
export class QueryInputElement extends LitElement {
  ...
  render() {
    ...
    return html`
      <div class="query-input absolute z-50 bg-slate-25 border rounded font-normal top-10 left-0">
        ...
        <div class="min-w-56 max-h-56 overflow-y-auto">
          ${list.map((item, i) => html`
            <a class="w-full text-left px-2 py-1 hover:bg-slate-100 flex justify-between cursor-pointer ${i === this.selected - 1 ? 'bg-blue-100' : ''}"
              ...
              ${!this.suggestions.length && Array.isArray(item) ? html`
...
export class FilterElement extends LitElement {
  ...
  #renderFilter = (node, path = '') => {
    ...
    if (['And', 'Or'].includes(node.tag)) {
      ...
      <div class="flex flex-wrap gap-2">
        ${node.contents.map((content, i) => html`
          ${this.#renderFilter(content, `${path}.contents[${i}]`)}
          ${i < node.contents.length - 1 ? html`
            <a class="bg-gray-200 text-xs px-2 py-1 rounded-full cursor-pointer inline-flex items-center gap-1" 
              @click=${() => this.#toggleAndOr(`${path}.tag`)}  data-path="${path}.tag">
              ${OPERATORS[node.tag]} 
    ...
    const renderEditableField = (fieldPath, value, editType, exprParts) => html`
      <div class="relative">
        <a class="p-1 cursor-pointer hover:bg-[#d5f0ff]" 
          @click=${() => { this.editingPath = fieldPath; this.requestUpdate(); }}>
          ${editType === 'value' ? JSON.stringify(value) : value}
        </a>
        ${this.editingPath === fieldPath ? html`
          <query-input-element
            .editContext=${{ editType, path: fieldPath, currentValue: value, exprParts }}
            @update-query=${this.#handleUpdateQuery}
  ...
  #closeOnClickOutside = e =>
    ...
    (this.editingPath = "") &&
    (this.newQuery = false);
  connectedCallback() { super.connectedCallback(); window.addEventListener('click', this.#closeOnClickOutside) }
