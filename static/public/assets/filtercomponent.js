        ...
        >
          ...
          <div class="flex flex-wrap gap-2">
            ...
            ${this.showFilterSearch
              ? html`<filter-suggestions></filter-suggestions>`
              ...
              : html`<button
                  ...
                  @click=${() =>
                    (this.showFilterSearch = !this.showFilterSearch)}
                  class="px-2 py-1 border rounded text-gray-500 hover:bg-gray-100"
                >
...
class Filter extends LitElement {
        ...
        >
          ${this.matches.map(
            (match) => html`
              <button
                type="button"
                class="match_buttons px-4 py-1 text-base text-left hover:bg-gray-100"
                @click=${(e) => {
  ...
  handleChange(val) {
    ...
    let filters = this.fields.filter(
      (v) => v.startsWith(this.inputVal) || this.inputVal.startsWith(v)
    );
    let auto_complete = [];
    filters.forEach((filter) => {
      ...
      if (!target_info) {
        ...
        if (key.result) {
          fetch(
            `/p/${this.projectId}/query_builder/autocomplete?category=${key.field}&prefix=.${key.prefix}`
          )
            .then((res) => res.json())
            .then((data) => {
          ...
          if (result) {
            fetch(
              `/p/${this.projectId}/query_builder/autocomplete?category=${result.key}&prefix=${result.prefix}`
            )
              .then((res) => res.json())
              .then((data) => {
                this.matches = data.map(
                  (d) => `${result.key} ${result.operator} "${d}"`
                );
              });
            return;
          } else {
      ...
      if (
        filter == this.inputVal.trim() ||
        filter.length > this.inputVal.length
      ) {
