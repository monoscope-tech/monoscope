-- Follow-up to 0094: swap now() → app_now() inside log_auto_resolve_activity
-- trigger body so the recorded activity timestamp honours the test clock when
-- tests fast-forward time. Original body defined in 0066.
...
BEGIN
  ...
  THEN
    ...
    SELECT i.id, 'auto_resolved', app_now()
    FROM apis.issues i
