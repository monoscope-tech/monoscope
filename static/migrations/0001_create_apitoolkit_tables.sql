...
CREATE TABLE IF NOT EXISTS user_auth_options
(
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  deleted BOOL NOT NULL DEFAULT 'f',
...
SELECT manage_updated_at('user_auth_options');
...
CREATE TYPE project_permissions AS ENUM ('admin', 'view', 'edit');
...
(
  ...
  permissions project_permissions NOT NULL DEFAULT 'view',
...
SELECT manage_updated_at('project_api_keys');
...
---
---
CREATE TABLE IF NOT EXISTS requests_dump
(
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    updated_at timestamp with time zone NOT NULL DEFAULT now(),
    id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
SELECT manage_updated_at('requests_dump');
...
SELECT manage_updated_at('fields');
...
(
  ...
  field_type text NOT NULL DEFAULT 'unknown'::field_type,
