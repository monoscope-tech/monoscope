      ...
      let trId = "aabbccdd11223344aabbccdd11223344"
          ...
          reqWith =
            mkSpanRequest
              ...
              []
              Nothing
              [ mkAttr "session.id" "sess-100"
              , mkAttr "user.id" "usr-200"
              , mkAttr "user.email" "u@test.com"
              , mkAttr "user.name" "uname"
              , mkAttr "user.full_name" "User Full"
              ]
              resource
              frozenTime
          reqWithout =
      ...
      let ts = addUTCTime (-300) now
          ...
          reqWith =
            mkSpanRequest
              ...
              []
              Nothing
              [mkAttr "session.id" "sess-backfill", mkAttr "user.id" "usr-backfill"]
              resource
              ts
      void $ OtlpServer.traceServiceExport tr.trLogger tr.trATCtx tr.trTracerProvider (Proto reqEmpty)
