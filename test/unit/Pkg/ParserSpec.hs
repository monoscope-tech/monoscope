...
spec = do
  describe "parseQueryToSQL" do
    ...
    it "query with now() time function" do
      ...
      let expected =
            [text|
      SELECT id,CASE WHEN RIGHT(TRIM('"' FROM CAST(to_json(timestamp at time zone 'UTC') AS VARCHAR)), 1) = 'Z' THEN TRIM('"' FROM CAST(to_json(timestamp at time zone 'UTC') AS VARCHAR)) ELSE TRIM('"' FROM CAST(to_json(timestamp at time zone 'UTC') AS VARCHAR)) || 'Z' END,context___trace_id,name,duration,resource___service___name,parent_id,CAST(EXTRACT(EPOCH FROM (start_time)) * 1000000000 AS BIGINT),( EXISTS ( SELECT 1 FROM jsonb_array_elements(events) elem WHERE elem->>'event_name' = 'exception' ) OR ( attributes->'apitoolkit'->>'errors' IS NOT NULL AND jsonb_array_length((attributes->'apitoolkit'->>'errors')::jsonb) > 0 )),to_json(summary),context___span_id FROM otel_logs_and_spans WHERE project_id='00000000-0000-0000-0000-000000000000' and ((timestamp = NOW())) ORDER BY timestamp desc limit 150|]
      normT query `shouldBe` normT expected
    ...
    it "basic query eq query" do
      ...
      let expected =
            [text|
      SELECT id,CASE WHEN RIGHT(TRIM('"' FROM CAST(to_json(timestamp at time zone 'UTC') AS VARCHAR)), 1) = 'Z' THEN TRIM('"' FROM CAST(to_json(timestamp at time zone 'UTC') AS VARCHAR)) ELSE TRIM('"' FROM CAST(to_json(timestamp at time zone 'UTC') AS VARCHAR)) || 'Z' END,context___trace_id,name,duration,resource___service___name,parent_id,CAST(EXTRACT(EPOCH FROM (start_time)) * 1000000000 AS BIGINT),( EXISTS ( SELECT 1 FROM jsonb_array_elements(events) elem WHERE elem->>'event_name' = 'exception' ) OR ( attributes->'apitoolkit'->>'errors' IS NOT NULL AND jsonb_array_length((attributes->'apitoolkit'->>'errors')::jsonb) > 0 )),to_json(summary),context___span_id FROM otel_logs_and_spans WHERE project_id='00000000-0000-0000-0000-000000000000' and ((method = 'GET')) ORDER BY timestamp desc limit 150|]
      normT query `shouldBe` normT expected
