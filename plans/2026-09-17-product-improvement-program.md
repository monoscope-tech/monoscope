...
| 2 | Alert to investigation is one path | Alert delivery and issue lifecycle flows exist. The legacy alert-detail route now gives current value, threshold and query, and hands off to the canonical monitor overview. Issue drill-down keeps the selected time range; its log handoff now quotes and carries the recorded service and environment, while trace-scoped reads remain exact. The issue timeline unions acknowledgement with alert/recovery episode events, and an integration regression proves both are visible together. | Retain the shared details-panel and time/scope hand-off contracts; extend the customer journey only when a concrete missing transition is identified. | Integration coverage traverses monitor evaluation, alert/issue state, evidence hand-off, acknowledgement and recovery; timeline rendering distinguishes user acknowledgement from measured recovery. |
...
| 4 | New projects reach first value | The sidebar checklist derives dashboard, monitor and successfully-sent test-notification milestones from live records; a privacy-safe, deduplicated project milestone ledger records the first verified ingest, dashboard, monitor and sent test notification. The onboarding integration journey proves all four records through the public handlers. The operator runbook `docs/runbooks/measure-activation-funnel.md` supplies cohort conversion and median-time measurement without user-level data. | Run the cohort report and use its conversion evidence to prioritize the next activation improvement. | Activation funnel report; empty/partial/error states remain actionable. |
| 5 | Environment and service scope are dependable | Sticky environment selection scopes Explorer, Issues, and RUM. Issues applies the session environment to both rows and service options, preserving the stored environment in its list projection. `/chart_data`, its stream, direct widget fetches, and dashboard server-prefill bind the authenticated session environment to generated KQL and cache identity. Charts also accept optional service scope, including cache identity. Raw-SQL widgets under either scope must include `{{query_ast_filters}}`, which expands to generated project, time, environment, service, and KQL predicates; complete and streaming reads otherwise return a visible error rather than leak another scope. API v1 metrics and both GET/POST event search expose explicit optional `environment` and `service`; omission intentionally means all matching rows and a two-environment/two-service ingestion fixture proves structural filtering for Explorer table and event-query paths. Monitor evaluation has no browser session, so API/YAML and browser-editor monitors persist optional environment and service scope, compile both as generated SQL predicates, and keep the customer KQL readable; widget-owned monitors retain their stored scope. Legacy monitors remain intentionally unscoped. A sticky `service` cookie, learned-facet picker, chart/dashboard binding, primary Explorer binding, RUM binding, and Issue-list binding are now implemented; auxiliary Explorer panels remain to be bound. | Bind the sticky service scope to Explorer's sessions/pattern panels, then add the two-environment/two-service browser fixture. | The fixture proves each browser-scoped surface excludes the other scope; URL attempts cannot override either authenticated scope. |
| 6 | Sessions and replay support investigation | Session/replay UI and panel cache exist. Monoscope now asks for router-matchable source columns: filtered `first_value(attributes___url___path ORDER BY timestamp)` and `MAX(attributes___user_agent___original)`, without renamed rollup columns. The paired TimeFusion change (`0d99fdce`) declares the same source-named measures and has a router regression. Session detail preserves replay, identity, landing page and raw telemetry context; browser-error occurrences also link into that workspace. | Deploy the paired TimeFusion spec and backfill, then compare raw and tier results over an agreed fixture and confirm the production route. | Deployed rollup route, raw-versus-tier result evidence, and existing session detail/empty/replay-only coverage. |
| 7 | Failure and recovery are visible and trustworthy | Expired-session fetch failures are fixed; `/chart_data` 504/500 are visible with a retry action, and the widget client preserves its rendered data through a recoverable failure. The issue timeline exposes alert episode history, acknowledgement and recovery as distinct events; its handler regression proves acknowledgement and measured recovery together. | Keep failure messages, retry affordances and original query/time context on every new data surface; add a customer-flow regression whenever a lifecycle transition gains a new presentation. | Widget tests cover visible failure, retry and recovery without duplicate fetches; issue handler coverage shows acknowledgement and recovery history together. |
| 8 | Billing matches product promises | Usage events are emitted, but attaching metered prices is an external billing-boundary decision; copy must not promise uncharged usage. The operator runbook is in `docs/runbooks/activate-metered-billing.md`. | At a recorded billing boundary, attach the approved prices; then update product/marketing copy and prove provider/webhook reconciliation. | Operator records attachment at a billing boundary; UI copy, subscription state and invoice fixture agree. |
